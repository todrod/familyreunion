    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4">
        <div className="w-full max-w-sm space-y-5">
          ...
          <Button onClick={unlockHost} className="h-10 w-full bg-[#d4a853] text-[#1c1208] hover:bg-[#c8553d] hover:text-[#f5ede0]">
            Unlock
          </Button>
          ...
          <>
            ...
            <div className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3">
              <div className="flex items-center gap-3">
                <span className={`inline-block h-2.5 w-2.5 rounded-full ${session.status === "question" ? "bg-green-500 animate-pulse" : session.status === "reveal" ? "bg-[#d4a853]" : session.status === "finished" ? "bg-rose-500" : "bg-zinc-500"}`} />
            ...
            <div className="flex flex-wrap gap-2">
              {session.status === "question" && (
                <Button onClick={() => doAction("reveal")} disabled={loading} className="gap-2 bg-[#d4a853] text-[#1c1208] hover:bg-[#c8553d] hover:text-[#f5ede0]">
                  <Eye className="h-4 w-4" /> Reveal Answer
                </Button>
              )}
              {session.status === "reveal" && (
                <Button onClick={() => doAction("next")} disabled={loading} className="gap-2 bg-[#d4a853] text-[#1c1208] hover:bg-[#c8553d] hover:text-[#f5ede0]">
                  <SkipForward className="h-4 w-4" />
                  {session.current_question + 1 >= session.total_questions ? "Finish Game" : "Next Question"}
                </Button>
              )}
              {session.status === "finished" && (
                <Button onClick={newSession} disabled={loading} className="gap-2 bg-[#d4a853] text-[#1c1208] hover:bg-[#c8553d] hover:text-[#f5ede0]">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {players.map((p) => (
                      <span key={p.player_name}
                        className="inline-flex items-center rounded-full border border-[#d4a853]/30 bg-[#d4a853]/10 px-3 py-1 text-sm font-medium text-foreground">
                        {p.player_name}
                      </span>
                    ))}
                  </div>
            ...
            {session.status === "finished" && (
              <div className="rounded-2xl border border-[#d4a853]/30 bg-[#d4a853]/5 px-6 py-8 text-center">
                <Trophy className="mx-auto h-10 w-10 text-[#d4a853]" />
              ...
              {addOpen && (
                <div className="rounded-xl border border-border bg-background p-4 space-y-3">
                  <input
                    ...
                    className="h-9 w-full rounded-lg border border-border bg-card px-3 text-sm text-foreground placeholder:text-muted-foreground focus:border-[#d4a853]/60 focus:outline-none focus:ring-2 focus:ring-[#d4a853]/20 transition-colors"
                  />
                    ...
                    {OPTION_LABELS.map((label, i) => (
                      <div key={label} className="flex items-center gap-1.5">
                        ...
                        <input
                          ...
                          className="h-8 flex-1 rounded-lg border border-border bg-card px-2 text-sm text-foreground placeholder:text-muted-foreground focus:border-[#d4a853]/60 focus:outline-none focus:ring-1 focus:ring-[#d4a853]/20 transition-colors"
                        />
                      </div>
                    ))}
                  </div>
