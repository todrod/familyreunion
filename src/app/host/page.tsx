    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4">
        <div className="w-full max-w-sm space-y-5">
          ...
          <Button onClick={unlock} className="h-10 w-full bg-[#c28e2b] text-[#14321f] hover:bg-[#bf5a33] hover:text-[#f6f1e2]">Unlock</Button>
        </div>
      </div>
    );
  }
      ...
      {!session ? (
        ...
        <div className="rounded-2xl border border-border bg-card p-6 space-y-5">
            ...
            <div className="flex gap-2 flex-wrap">
              {INTERVAL_OPTIONS.map(s => (
                <button key={s} onClick={() => setCallInterval(s)}
                  className={`rounded-lg border px-4 py-1.5 text-sm font-medium transition-colors ${callInterval === s ? "border-[#c28e2b]/50 bg-[#c28e2b]/10 text-[#c28e2b]" : "border-border text-muted-foreground hover:border-[#c28e2b]/30 hover:text-foreground"}`}>
                  {s}s
                </button>
              ))}
            </div>
          ...
          <Button onClick={startGame}
            className="w-full gap-2 bg-[#c28e2b] text-[#14321f] hover:bg-[#bf5a33] hover:text-[#f6f1e2]">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 space-y-5">
          <div className="flex items-center justify-between">
            ...
            <button onClick={endGame}
              className="text-xs text-muted-foreground hover:text-[#bf5a33] transition-colors flex items-center gap-1">
              ...
              <div className="flex flex-wrap gap-2">
                {players.map(p => (
                  <span key={p.player_name}
                    className="rounded-full border border-[#c28e2b]/30 bg-[#c28e2b]/10 px-3 py-1 text-sm text-foreground">
                    {p.player_name}
                  </span>
                ))}
              </div>
        ...
        <>
          ...
          <div className="rounded-2xl border-2 border-[#c28e2b]/40 bg-gradient-to-br from-[#1d4d33] to-[#14321f] p-6 text-center">
            {lastWord ? (
              <>
                <p className="text-xs uppercase tracking-widest text-[#b39357] mb-3">Current Word</p>
                <p className="text-4xl md:text-5xl font-black text-[#c28e2b] leading-tight">{lastWord}</p>
                {countdown !== null && countdown > 0 && (
                  <div className="mt-4 flex items-center justify-center gap-1.5 text-[#b39357]">
              ...
              <p className="text-[#b39357] text-lg">Press &ldquo;Call Word&rdquo; to begin!</p>
            )}
          </div>
          ...
          <Button
            ...
            className={`w-full gap-2 text-base h-12 transition-all ${readyForNext && lastWord ? "bg-green-600 hover:bg-green-500 text-white ring-2 ring-green-400/50" : "bg-[#c28e2b] text-[#14321f] hover:bg-[#bf5a33] hover:text-[#f6f1e2]"}`}
          >
      ...
      <div className="flex items-center justify-between">
        ...
        {!session && (
          <Button onClick={newSession} disabled={loading || questions.length === 0}
            className="gap-2 bg-[#c28e2b] text-[#14321f] hover:bg-[#bf5a33] hover:text-[#f6f1e2]">
        ...
        <>
          ...
          <div className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3">
            <div className="flex items-center gap-3">
              <span className={`h-2.5 w-2.5 rounded-full ${session.status === "question" ? "bg-green-500 animate-pulse" : session.status === "reveal" ? "bg-[#c28e2b]" : session.status === "finished" ? "bg-rose-500" : "bg-zinc-500"}`} />
          ...
          {session.status === "waiting" && (
            <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
              ...
              {players.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {players.map(p => (
                    <span key={p.player_name} className="rounded-full border border-[#c28e2b]/30 bg-[#c28e2b]/10 px-3 py-1 text-sm font-medium">
                      {p.player_name}
                    </span>
                  ))}
                </div>
            ...
            {addOpen && (
              <div className="rounded-xl border border-border bg-background p-4 space-y-3">
                <input type="text" placeholder="Question text…" value={newQ.question}
                  ...
                  className="h-9 w-full rounded-lg border border-border bg-card px-3 text-sm focus:border-[#c28e2b]/60 focus:outline-none focus:ring-2 focus:ring-[#c28e2b]/20 transition-colors" />
                  ...
                  {OPTION_LABELS.map((label, i) => (
                    <div key={label} className="flex items-center gap-1.5">
                      ...
                      <input type="text" placeholder={`Option ${label}…`} value={newQ.options[i]}
                        ...
                        className="h-8 flex-1 rounded-lg border border-border bg-card px-2 text-sm focus:border-[#c28e2b]/60 focus:outline-none focus:ring-1 focus:ring-[#c28e2b]/20 transition-colors" />
                    </div>
                  ))}
                </div>
