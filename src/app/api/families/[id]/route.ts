...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await pool.query(
    `UPDATE families SET family_name=?, contact_name=?, rooms_requested=?, nights=?, attendees=?, notes=? WHERE id=?`,
    [family_name, contact_name, Number(rooms_requested), Number(nights), attendees, notes, id]
  );
