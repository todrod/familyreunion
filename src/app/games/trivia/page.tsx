      ...
      <div className="mx-auto max-w-lg px-4 py-8">
        ...
        {!playerName ? (
          <div className="flex flex-col items-center gap-6 py-10">
            ...
            <div className="w-full max-w-sm space-y-3">
              ...
              <input
                ...
                className="h-14 w-full rounded-2xl border border-border bg-card px-4 text-base text-foreground placeholder:text-muted-foreground focus:border-[#c28e2b]/60 focus:outline-none focus:ring-2 focus:ring-[#c28e2b]/20 transition-colors"
                autoFocus
              />
              <Button onClick={joinGame} className="h-14 w-full rounded-2xl bg-[#c28e2b] text-[#14321f] hover:bg-[#bf5a33] hover:text-[#f6f1e2] text-lg font-semibold">
                Join Game
              </Button>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center gap-5 py-16 text-center">
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#c28e2b]/10 border border-[#c28e2b]/30">
              <Users className="h-7 w-7 text-[#c28e2b]" />
            </div>
            <div>
            ...
            <div className="rounded-xl border border-border bg-card px-6 py-4 text-center">
              <p className="text-3xl font-bold text-[#c28e2b]">{playerCount}</p>
          ...
          <div className="space-y-6 py-6">
            <div className="text-center">
              <Trophy className="mx-auto h-12 w-12 text-[#c28e2b]" />
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span className="rounded-full border border-border px-2.5 py-1 text-[#c28e2b]">{session.question.category}</span>
            ...
            <div className="grid grid-cols-1 gap-3">
              {session.question.options.map((opt, i) => {
                ...
                let cls = "relative flex items-center gap-3 rounded-2xl border px-5 py-5 text-left text-base font-medium text-white transition-all ";
                if (isReveal) {
                  cls += isCorrect ? OPTION_REVEAL_CORRECT + " border-green-400 " : OPTION_REVEAL_WRONG + " border-transparent ";
                } else if (isSelected) {
                  cls += "bg-[#c28e2b] border-[#c28e2b] text-[#14321f] ";
                } else if (submitted) {
                ...
                return (
                  ...
                  >
                    <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/20 text-sm font-bold">{label}</span>
...
function Leaderboard({ entries, highlight, compact }: { entries: LeaderboardEntry[]; highlight: string; compact?: boolean }) {
  ...
  return (
    <div className={`space-y-2 ${compact ? "" : "rounded-2xl border border-border bg-card p-5"}`}>
      ...
      {entries.map((entry, i) => (
        <div key={entry.name} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 ${entry.name === highlight ? "bg-[#c28e2b]/15 border border-[#c28e2b]/30" : "bg-muted/40"}`}>
